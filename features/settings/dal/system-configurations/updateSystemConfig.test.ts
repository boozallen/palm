...
describe('updateSystemConfig', () => {
  ...
  it('should update the document library Document Upload provider ID config successfully', async () => {
    (db.systemConfig.updateMany as jest.Mock).mockResolvedValue({ count: 1 });

    const documentLibraryDocumentUploadProviderId = 'SomeOtherProviderId';
    const response = await updateSystemConfig(
      SystemConfigFields.DocumentLibraryDocumentUploadProviderId,
      documentLibraryDocumentUploadProviderId
    );

    expect(db.systemConfig.updateMany).toHaveBeenCalledWith({
      data: {
        [SystemConfigFields.DocumentLibraryDocumentUploadProviderId]: documentLibraryDocumentUploadProviderId.length ? documentLibraryDocumentUploadProviderId : null,
      },
    });
    expect(response).toEqual({
      count: 1,
      updatedField: SystemConfigFields.DocumentLibraryDocumentUploadProviderId,
      updatedValue: documentLibraryDocumentUploadProviderId,
    });
    expect(logger.info).toHaveBeenCalledWith(
      `System configuration ${SystemConfigFields.DocumentLibraryDocumentUploadProviderId} updated.`
    );
  });
