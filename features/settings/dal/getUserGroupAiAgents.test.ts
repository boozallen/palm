...
describe('getUserGroupAiAgents', () => {
  ...
  it('should return only enabled agents associated with the user group', async () => {
    ...
    expect(result).toContainEqual(mockAgents[0]);
    expect(result).toContainEqual(mockAgents[2]);
    expect(result).not.toContainEqual(mockAgents[1]); 
  });
