...
describe('useRegenerateResponse', () => {
  ...
  describe('Mutation Call', () => {
    ...
    it('calls the retryMessage mutation with chat id, prompt, and documentLibraryEnabled', async () => {
      const { result } = renderHook(useRegenerateResponse);
  ...
  describe('Error handling', () => {
    ...
    it('sets error if there is no chatId', () => {
      createChatProviderStub({ chatId: null, setRegeneratingResponse: setRegeneratingResponseMock, documentLibraryEnabled });
      const { result } = renderHook(useRegenerateResponse);
