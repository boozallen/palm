...
export default async function getContentFromKbs(ctx: ContextType, input: getContentInput): Promise<KbResults> {
  let citations: { citation: string, knowledgeBaseId: string }[] = [];
  let failedKbs: string[] = [];
  let resultsArray: { content: string, score: number, citation?: string, knowledgeBaseId: string }[] = [];
    ...
    try {
      ...
      kbContent.results.forEach((result: SearchResult) => {
        resultsArray.push({
          ...
          score: result.score,
          citation: result.citation?.label,
  ...
  const content = resultsArray.map(result => `Content:${result.content} (Citation: ${result.citation ?? 'No citation'}, Score: ${result.score})`).join('\n');

  return { content, citations, failedKbs };
}
