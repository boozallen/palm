...
jest.mock('@/features/chat/knowledge-bases/addKbContentToMessage');
...
describe('add-message', () => {
  ...
  beforeEach(() => {
    ...
    (addKbContentToMessage as jest.Mock).mockReturnValue(kbEnhancedMessage);
    ...
    (extractArtifactsFromMessage as jest.Mock).mockReturnValue({
      extractedArtifacts: mockExtractedArtifacts,
  ...
  it('does call knowledge base functions', async () => {
    ...
    expect(addKbContentToMessage).toHaveBeenCalledWith(
      mockMessage,
      kbResultsMock.content
    );
  });
});
