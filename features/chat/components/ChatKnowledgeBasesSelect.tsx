...
export default function ChatKnowledgeBasesSelect() {
  ...
  const preselectedKnowledgeBasesIds = useMemo(() => {
    if (params?.knowledgebaseids) {
      return params.knowledgebaseids.split(',');
    } else if (params?.knowledgebaseids === '') {
      return [];
    }
  ...
  }, [preselectedKnowledgeBases, params?.knowledgebaseids, params?.chatId]);
