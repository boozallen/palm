  ...
  const remainingCitationsHovercardContent = (
    <Stack spacing={'sm'}>
      {remainingCitations.map((citation) => (
        <div key={`${citation.citation}-${citation.knowledgeBaseLabel}`}>
          <Title weight='normal' color='gray.6' order={4} truncate>
            {citation.knowledgeBaseLabel}
          </Title>
          <Text size='xs'>
            {citation.citation}
          </Text>
    ...
    <Avatar.Group spacing='sm' pb='md'>
      {displayedCitations.map((citation, index) => (
        <div key={`${citation.citation}-${citation.knowledgeBaseLabel}`}>
          <HoverCard shadow='md' withArrow position='top'>
            ...
            <HoverCard.Dropdown style={{ maxWidth: '50%', maxHeight: '25%', overflow: 'auto' }}>
              <Title weight='normal' color='gray.6' order={4} truncate>
                {citation.knowledgeBaseLabel}
              </Title>
              <Text size='xs'>
      ...
      {remainingCitationsCount > 0 && (
        <HoverCard shadow='md' withArrow position='top'>
          ...
          <HoverCard.Dropdown style={{ maxWidth: '50%', maxHeight: '45%', overflow: 'auto' }}>
